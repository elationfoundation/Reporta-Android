            ...
            Utility.getGraphMeRequestWithCacheAsync(accessToken, new Utility.GraphMeRequestWithCacheCallback() {
                        ...
                        public void onSuccess(JSONObject userInfo) {
                            try {
                                ...
                                loginClient.complete(LoginClient.Result.createErrorResult(loginClient.getPendingRequest(), "Caught exception", ex.getMessage()));
                            }
                        }

                        @Override
                        public void onFailure(FacebookException error) {
                            loginClient.complete(LoginClient.Result.createErrorResult(loginClient.getPendingRequest(), "Caught exception", error.getMessage()));
                        }
                    });
        } else {
            onComplete(request, result);
