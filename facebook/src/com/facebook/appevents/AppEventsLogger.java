...
public class AppEventsLogger {
    ...
    private static FlushStatistics buildAndExecuteRequests(FlushReason reason, Set<AccessTokenAppIdPair> keysToFlush) {
        ...
        for (AccessTokenAppIdPair accessTokenAppId : keysToFlush) {
            ...
            GraphRequest request = buildRequestForSession(accessTokenAppId, sessionEventsState, limitEventUsage, flushResults);
            if (request != null) {
    ...
    private static GraphRequest buildRequestForSession(final AccessTokenAppIdPair accessTokenAppId, final SessionEventsState sessionEventsState, final boolean limitEventUsage, final FlushStatistics flushState) {
            ...
            synchronized (staticLock) {
                if (anonymousAppDeviceGUID == null) {
                    ...
                    if (anonymousAppDeviceGUID == null) {
                        ...
                        context.getSharedPreferences(APP_EVENT_PREFERENCES, Context.MODE_PRIVATE).edit().putString("anonymousAppDeviceGUID", anonymousAppDeviceGUID).apply();
                    }
                }
            }
        }
    ...
    private static class AccessTokenAppIdPair implements Serializable {
        ...
        @Override
        public int hashCode() {
            return (accessTokenString == null ? 0 : accessTokenString.hashCode()) ^ (applicationId == null ? 0 : applicationId.hashCode());
        }
        ...
        public boolean equals(Object o) {
            ...
            return Utility.areObjectsEqual(p.accessTokenString, accessTokenString) && Utility.areObjectsEqual(p.applicationId, applicationId);
        }
    ...
    static class SessionEventsState {
        ...
        private void populateRequest(GraphRequest request, int numSkipped, JSONArray events, boolean limitEventUsage) {
            ...
            try {
                publishParams = AppEventsLoggerUtility.getJSONObjectForGraphAPICall(AppEventsLoggerUtility.GraphAPIActivityType.CUSTOM_APP_EVENTS, attributionIdentifiers, anonymousAppDeviceGUID, limitEventUsage, applicationContext);
    ...
    static class AppEvent implements Serializable {
        ...
        private void validateIdentifier(String identifier) throws FacebookException {
            ...
            if (identifier == null || identifier.length() == 0 || identifier.length() > MAX_IDENTIFIER_LENGTH) {
                ...
                throw new FacebookException(String.format(Locale.ROOT, "Identifier '%s' must be less than %d characters", identifier, MAX_IDENTIFIER_LENGTH));
            }
            ...
            if (!alreadyValidated) {
                if (identifier.matches(regex)) {
                    ...
                    throw new FacebookException(String.format("Skipping event named '%s' due to illegal name - must be " +
                            "under 40 chars and alphanumeric, _, - or space, and " +
                            "not start with a space or hyphen.", identifier));
                }
            }
        }
        ...
        public String toString() {
            return String.format("\"%s\", implicit: %b, json: %s", jsonObject.optString("_eventName"), isImplicit, jsonObject.toString());
        }
    ...
    static class PersistedAppSessionInfo {
        ...
        private static void onTimeSpentDataUpdate() {
            if (!hasChanges) {
                hasChanges = true;
                backgroundExecutor.schedule(appSessionInfoFlushRunnable, FLUSH_APP_SESSION_INFO_IN_SECONDS, TimeUnit.SECONDS);
            }
        }
    }
