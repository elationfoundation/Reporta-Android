...
final class AccessTokenManager {
    ...
    private boolean shouldExtendAccessToken() {
        ...
        return currentAccessToken.getSource().canExtendToken() && now - lastAttemptedTokenExtendDate.getTime() > TOKEN_EXTEND_RETRY_SECONDS * 1000 && now - currentAccessToken.getLastRefresh().getTime() > TOKEN_EXTEND_THRESHOLD_SECONDS * 1000;
    }
    ...
    private void refreshCurrentAccessTokenImpl() {
        ...
        batch.addCallback(new GraphRequestBatch.Callback() {
            ...
            public void onBatchCompleted(GraphRequestBatch batch) {
                if (getInstance().getCurrentAccessToken() == null || getInstance().getCurrentAccessToken().getUserId() != accessToken.getUserId()) {
                    return;
                }
                try {
