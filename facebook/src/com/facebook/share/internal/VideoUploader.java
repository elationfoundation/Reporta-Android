...
public class VideoUploader {
    ...
    private static void issueResponse(final UploadContext uploadContext, final FacebookException error, final String videoId) {
    ...
    private static byte[] getChunk(UploadContext uploadContext, String chunkStart, String chunkEnd) throws IOException {
        ...
        while ((len = uploadContext.videoStream.read(buffer)) != -1) {
            ...
            if (chunkSize == 0) {
                ...
                logError(null, "Error reading video chunk. Expected buffer length - '%d'. Actual - '%d'.", chunkSize + len, len);
                return null;
            }
        }
        ...
        private void initialize() throws FileNotFoundException {
            ...
            try {
                if (Utility.isFileUri(videoUri)) {
                    fileDescriptor = ParcelFileDescriptor.open(new File(videoUri.getPath()), ParcelFileDescriptor.MODE_READ_ONLY);
    ...
    private static abstract class UploadWorkItemBase implements Runnable {
        ...
        protected void executeGraphRequestSynchronously(Bundle parameters) {
            GraphRequest request = new GraphRequest(uploadContext.accessToken, String.format(Locale.ROOT, "%s/videos", uploadContext.graphNode), parameters, HttpMethod.POST, null);
        ...
        private boolean attemptRetry(int errorCode) {
            if (completedRetries < MAX_RETRIES_PER_PHASE && getTransientErrorCodes().contains(errorCode)) {
                int delay = RETRY_DELAY_UNIT_MS * (int) Math.pow(RETRY_DELAY_BACK_OFF_FACTOR, completedRetries);
