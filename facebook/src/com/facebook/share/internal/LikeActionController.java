...
public class LikeActionController {
    ...
    private static void storeObjectIdForPendingController(String objectId) {
        ...
        appContext.getSharedPreferences(LIKE_ACTION_CONTROLLER_STORE, Context.MODE_PRIVATE).edit().putString(LIKE_ACTION_CONTROLLER_STORE_PENDING_OBJECT_ID_KEY, objectIdForPendingController).apply();
    }
    ...
    public interface CreationCallback {
        ...
        void onComplete(LikeActionController likeActionController, FacebookException error);
    }
    ...
    private class GetPageIdRequestWrapper extends AbstractRequestWrapper {
        ...
        @Override
        protected void processSuccess(GraphResponse response) {
            JSONObject results = Utility.tryGetJSONObjectFromResponse(response.getJSONObject(), objectId);
            if (results != null) {
        ...
        @Override
        protected void processError(FacebookRequestError error) {
            Logger.log(LoggingBehavior.REQUESTS, TAG, "Error getting the FB id for object '%s' with type '%s' : %s", objectId, objectType, error);
        }
    }
